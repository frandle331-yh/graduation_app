# frozen_string_literal: true

# ダッシュボード表示に必要なデータを集約する
class DashboardPresenter
  PERIOD_LABELS = { "week" => "今週", "month" => "今月", "all" => "全期間" }.freeze

  attr_reader :user, :household, :period

  def initialize(user:, household:, period:)
    @user = user
    @household = household
    @period = period.presence_in(%w[week month all]) || "week"
  end

  def period_label
    PERIOD_LABELS[@period]
  end

  def streak
    @streak ||= StreakCalculator.new(user).current_streak
  end

  def weekly_log_days
    @weekly_log_days ||= user.weekly_log_days
  end

  def total_minutes
    @total_minutes ||= period_scope.sum(:minutes)
  end

  def total_count
    @total_count ||= period_scope.count
  end

  def category_summaries
    @category_summaries ||= period_scope.group(:category).sum(:minutes)
  end

  def daily_summaries
    @daily_summaries ||= period_scope
      .group(:performed_on)
      .sum(:minutes)
      .sort_by { |date, _| date }
      .to_h
  end

  def weekly_summary
    @weekly_summary ||= build_weekly_summary
  end

  def badges
    @badges ||= BadgeCalculator.new(user).all_badges
  end

  def earned_badge_count
    badges.count { |b| b[:earned] }
  end

  def calendar
    @calendar ||= build_calendar
  end

  def contribution
    @contribution ||= build_contribution
  end

  private

  def today
    @today ||= Time.zone.today
  end

  def base_scope
    @base_scope ||= HouseworkLog.accessible_by(user, household)
  end

  def period_scope
    @period_scope ||=
      case @period
      when "month"
        base_scope.where(performed_on: today.beginning_of_month..today.end_of_month)
      when "all"
        base_scope
      else
        base_scope.where(performed_on: today.beginning_of_week..today.end_of_week)
      end
  end

  def build_weekly_summary
    this_week = today.beginning_of_week..today.end_of_week
    last_week = (today.beginning_of_week - 7)..(today.end_of_week - 7)

    this_scope = base_scope.where(performed_on: this_week)
    last_scope = base_scope.where(performed_on: last_week)

    days_elapsed = [ (today - today.beginning_of_week).to_i + 1, 1 ].max
    this_minutes = this_scope.sum(:minutes)

    {
      count: this_scope.count,
      minutes: this_minutes,
      last_count: last_scope.count,
      last_minutes: last_scope.sum(:minutes),
      top_category: this_scope.group(:category).count.max_by { |_, v| v }&.first,
      avg_minutes_per_day: (this_minutes.to_f / days_elapsed).round(0)
    }
  end

  def build_calendar
    first = today.beginning_of_month
    last  = today.end_of_month

    {
      month_label: today.strftime("%Y年%-m月"),
      first: first,
      last: last,
      today: today,
      data: user.housework_logs
              .where(performed_on: first..last)
              .group(:performed_on)
              .count
    }
  end

  def build_contribution
    return nil unless household

    minutes_by_user_id = period_scope.group(:user_id).sum(:minutes)
    users_by_id = household.users.where(id: minutes_by_user_id.keys).index_by(&:id)

    minutes_by_user = minutes_by_user_id
      .transform_keys { |uid| users_by_id[uid] }
      .compact

    total = total_minutes.to_i
    rates_by_user = minutes_by_user.transform_values do |minutes|
      total.zero? ? 0 : ((minutes.to_f / total) * 100).round(1)
    end

    { minutes_by_user: minutes_by_user, rates_by_user: rates_by_user }
  end
end
